...
class SpendingTracker(tk.Tk):
    def __init__(self):
        ...
        # Adiciona um fundo degradê verde claro à janela
        ...
        self.frame_entrada = tk.Frame(self, bg="#a3d2a5")  # Define o fundo do frame como verde claro
        ...
        self.botao_adicionar = tk.Button(self.frame_entrada, text="Adicionar Despesa", command=self.dialogo_adicionar_despesa)
    ...
    def dialogo_adicionar_despesa(self):
        ...
        # Adiciona º ao lado do número se houver parcelas
        parcelas_acordo = f"{parcelas}º" if parcelas else ""

        self.despesas.append((self.combo_ano.get(), self.combo_mes.get(), despesa, valor, vencimento, parcelas_acordo, observacao))
    ...
    def excluir_despesa(self):
        # Abre a caixa de diálogo para selecionar o arquivo existente
        ...
        if arquivo_existente:
            # Carrega as despesas do arquivo existente
            self.carregar_despesas(arquivo_existente)

            # Abre a caixa de diálogo para selecionar a despesa a ser excluída
            despesa_selecionada = simpledialog.askinteger("Excluir Despesa", "Digite o número da despesa a ser excluída:", minvalue=1, maxvalue=len(self.despesas))

            if despesa_selecionada is not None:
                index = despesa_selecionada - 1  # O índice começa em 0
    ...
    def carregar_despesas(self, arquivo_existente):
        # Carrega despesas de um arquivo existente
        try:
            planilha = openpyxl.load_workbook(arquivo_existente)
            folha = planilha.active
            self.despesas = [tuple(row) for row in folha.iter_rows(values_only=True)][1:]  # Ignora o cabeçalho
            planilha.close()
    ...
    def salvar_no_excel(self):
        ...
        # Adiciona a formatação R$ à coluna de valores
        for row_num in range(2, folha.max_row + 1):
            folha[f'D{row_num}'].number_format = 'R$#,##0.00'

        # Adiciona linhas de grade à planilha
